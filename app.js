...
function renderMedia(force=false){const task=currentTask();if(!task)return;const preview=$('#mediaPreview');const signature=[task.id,task.mediaUrl,task.mediaUrl?'':task.status,task.mediaUrl?'':task.progress,task.errorCode].join('|');
 ...
 const download=$('#downloadBtn');download.innerHTML=icon('download')+(task.mediaType==='image'?'下载图片':'下载视频');download.setAttribute('aria-disabled',String(!task.mediaUrl));if(task.mediaUrl){download.href=task.mediaUrl+'?download=1';download.download='';}else download.removeAttribute('href');
...
function renderField(force=false){const task=currentTask();if(!task)return;const field=state.field,value=task[field]||{},key=draftKey(task.id,field);$$('.result-tab').forEach(b=>{b.classList.toggle('active',b.dataset.field===field);b.setAttribute('aria-selected',String(b.dataset.field===field));});const text=state.dirty.has(key)?state.dirty.get(key):(value.text||'');if(force||document.activeElement!==$('#textResult')&&!state.dirty.has(key))$('#textResult').value=text;
 $('#fieldStatus').textContent=value.status||'待提取';$('#wordCount').textContent=`${$('#textResult').value.length} 字`;$('#fieldHint').textContent=fieldHint(task,field);$('#saveStatus').textContent=state.dirty.has(key)?'有未保存的修改':value.edited?'修改已保存':'修改后自动保存';$('#textResult').disabled=busyStates.includes(value.status)||state.extracting.has(task.id);$('#textResult').placeholder=busyStates.includes(value.status)?'正在识别，请稍候…':value.status==='已完成'?'未识别到清晰文字，你也可以手动补充。':'提取完成后，文字会出现在这里。';
...
function refresh(afterChange=false){
 ...
 refreshRunning=(async()=>{try{const before=currentTask()?.status;const data=await api('/api/tasks');state.tasks=data.tasks;state.online=true;$('#connection').classList.add('online');$('#connection').innerHTML='<i></i>服务已连接';renderLists();if(state.current&&getTask(state.current)){renderMedia();renderField();if(currentTask().error)setOperation(currentTask().error);else if(busyStates.includes(currentTask().status))setOperation(statusOf(currentTask())+'…');else if(busyStates.includes(before))setOperation('');}else if(state.current){state.current=null;$('#resultSection').hidden=true;$('#features').hidden=false;}runAutoExtract();}catch{$('#connection').classList.remove('online');$('#connection').innerHTML='<i></i>服务未连接';state.online=false;}})().finally(()=>{refreshRunning=null;});
...
function renderDeployment(){
 ...
 const available=['speech','ocr'].filter(key=>state.health?.[key]);
 $('.auto-extract span').textContent=available.length?'同时提取'+available.map(key=>key==='speech'?'口播':'画面文字').join('与'):'文字识别暂不可用';
 $('#autoExtract').disabled=!available.length;
...
function renderServices(){const h=state.health,net=h?.network;const networkLabel=net?.state==='connected'?'HTTPS 可达':net?.state==='access_denied'?'联网被拒绝':net?.state==='checking'?'检查中':net?.state==='unreachable'?'连接失败':'未检查';const rows=[['任务服务',state.online?'已连接':'未连接','上传、预览、文字保存与导出',state.online],['平台解析器',h?.download?'已安装':'未安装','具体视频以获取结果为准',h?.download],['平台连接',networkLabel,net?.message||'尚未检查后台服务连接',net?.state==='connected'],['画面文字识别',h?.ocr?'已就绪':h&&!h.checked?'检查中':'未就绪',h?.ocrEngine||'Windows 中文 OCR',h?.ocr],['语音识别',h?.speech?'已就绪':h&&!h.checked?'检查中':'未就绪',h?.speechEngine||'检查识别组件',h?.speech]];$('#serviceList').innerHTML=rows.map(([name,label,detail,ready])=>`<div class="service-row"><span><strong>${escape(name)}</strong><small>${escape(detail)}</small></span><span class="badge ${ready?'':name==='平台连接'&&net?.state==='access_denied'?'failed':'expired'}">${escape(label)}</span></div>`).join('');}
...
async function extract(keys,taskId=state.current){
 ...
 if(!requested.length){toast('识别组件尚未就绪');return;}
...
async function saveOne(key,text){
 ...
 const promise=(async()=>{
  ...
  try{
   ...
   if(state.current===id&&state.field===field)$('#saveStatus').textContent=state.dirty.has(key)?'正在保存…':'修改已保存';
...
function runAutoExtract(){
 ...
 for(const id of state.autoPending){const task=getTask(id);if(!task||task.status==='失败'){state.autoPending.delete(id);persistAuto();continue;}if(!task.mediaUrl||busy(task))continue;
  state.autoPending.delete(id);persistAuto();const keys=['speech','ocr'].filter(k=>state.health[k]&&(task.mediaType!=='image'||k==='ocr')&&!task[k]?.text&&!task[k]?.edited&&!state.dirty.has(draftKey(id,k))&&task[k]?.status!=='已完成');
